...
const OtherProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`https://orm-mat-be.onrender.com/api/products?page=${page}`);
  ...
  return (
        ...
        <>
          ...
          {products.length ===0 ? <Spinner color='success'></Spinner> :<>
            {products.map(product => (
              <Link to={`/product/${product._id}`} key={product._id} className=" my-5">
              <Card
                style={{
                  width: "18rem",
                  height: "300px",
              ...
              >
                ...
                <CardBody className=" bg rounded-bottom p-0">
                  <CardTitle>
                    <h6 className="text mt-3 text-center text rounded-2 m-2">
                      {product.name}
                    </h6>
                  </CardTitle>
                  {/* <h6 className="text mt-3 text-center mb-2">price : $ {product.price}</h6> */}
    ...
    Buy Now
          ...
          <div className='d-flex align-items-center justify-content-center flex-wrap mt-2 mb-5'>
            <button
            className='title p-2 rounded-2 border-0 text mx-3'
            ...
            <button
              className='title p-2 mx-3 rounded-2 border-0 text'
